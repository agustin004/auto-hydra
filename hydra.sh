...
#Configuracion
read -p "Por favor, ingresa el IP objetivo: " target
read -p "Ingresa el usuario : " user    #Nombre de usuario
read -p "Ingresa el diccionario: " diccionario

#Comprobacion dependencias
if ! command -v hydra &> /dev/null; then
    echo "Hydra no esta instalado. Instalalo con sudo apt-get install hydra" && exit
else 
    echo "Hydra esta instalado"
fi

if [ ! -f "$diccionario" ]; then
    echo -e "${rojo}El diccionario de passwords '$diccionario' no existe, debes ponerlo dentro del directorio actual${sin_color}"
    exit
fi
